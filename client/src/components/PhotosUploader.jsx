    ...
    return (
        <>
            <div className="flex gap-2">
                <input type="text" placeholder={'Adding using link ....jpg'} value={photoLink} onChange={event => setPhotoLink(event.target.value)}/>
            ...
            <div className="mt-2 gap-2 grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6">
                {addedPhotos.length > 0 && addedPhotos.map(link => (
                    <div className="h-32 relative flex" key={link}>
                        ...
                        <button onClick={(ev) => selectMainPhoto(ev, link)} className=' cursor-pointer absolute bottom-1 left-1 text-white bg-black py-2 px-3 bg-opacity-50 rounded-2xl'>
                            {link == addedPhotos[0] && (
